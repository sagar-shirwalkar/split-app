...
async function deploy() {
  ...
  if (!app) {
    app = (await rest("POST", "table/sys_app", { name: appMeta.name, scope: "x_snc_split", description: appMeta.description || "" })).result;
  ...
  for (const file of scriptFiles) {
    ...
    try {
      ...
      if (existing) {
        ...
        await rest("POST", "table/sys_script_include", { name: name, api_name: "x_snc_split." + name, script: script, access: "package_private" });
  ...
  if (wsDefRec) {
    ...
    // Update service_id to match api_path so the API is at /api/x_snc_split/... (minus instance ID prefix)
    await rest("PATCH", "table/sys_ws_definition/" + wsDefId, {
      service_id: "x_snc_split",
      active: true,
    }).catch(function() {});
    ...
    apiBasePath = updated.result && updated.result.base_uri ? updated.result.base_uri : "/api/x_snc_split";
    console.log("  Found: " + wsDefId + " (API at " + apiBasePath + ")");
  } else {
    const created = await rest("POST", "table/sys_ws_definition", { name: wsDef.name, service_id: "x_snc_split", active: true });
    wsDefId = created.result.sys_id;
    apiBasePath = "/api/x_snc_split";
